...
# Argument wejściowy nie może zostać przekonwertowany na liczbę całkowitą
function numberFormatException(str::String)
  @printf(STDERR, "\nBłąd: \"%s\" nie jest liczbą całkowitą\n\n", str)
  quit()
end

# Wyróżniony indeks jest nieprawidłowy
function chosenIndexOutOfBounds(vertex::Int64, n::Int64)
  @printf(STDERR, "\nBłąd: wybrany indeks wierzchołka (%d) jest spoza zakresu {1...%d}\n\n", vertex, n)
end

# Komunikat wyświetlany w przypadku podania nieprawidłowych argumentów wejściowych
function helloMessage()
  @printf(STDERR, "\nużycie:\n\t./main <In >Out k\ngdzie:\n\tIn: ścieżka do pliku wejściowego\n\tOut: ścieżka do pliku wyjściowego\n\tk: indeks wyróżnionego wierzchołka\n\n")
end
